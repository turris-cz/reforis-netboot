...
init-langs: create-messages
	for lang in $(LANGS); do \
		...
		-i reforis_netboot/translations/messages.pot \
		-d reforis_netboot/translations/ -l $$lang \
	; done

.PHONY: create-messages
create-messages: venv
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_netboot/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages: venv
	$(VENV_BIN)/pybabel update -i ./reforis_netboot/translations/messages.pot -d ./reforis_netboot/translations --update-header-comment
